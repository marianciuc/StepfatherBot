...
const { welcomeEntity } = require( '../entity/index' )
...
const get = ( member, { type } ) => {
    welcomeEntity.findOne( { guildId: member.guild.id }, ( err, document ) => {
            ...
            bot.channels.fetch( document.channel.id ).then( channel => {
                ...
                loadImage( `${document.image}` ).then( image => {
                    ...
                    loadImage( `${member.user.avatarURL( { format: "png" } )}` ).then( image => {
                        ...
                        } ).catch( error => debug.log( error, __dirname + __filename ) );
                    } ).catch( error => debug.log( error, __dirname + __filename ) );
                } ).catch( error => debug.log( error, __dirname + __filename ) );
            } ).catch( error => debug.log( error, __dirname + __filename ) );
        }
...
const create = ( guild, channel, url ) => {
    return new Promise( ( resolve => {
        ...
        welcomeEntity.findOneAndUpdate( { guildId: guild.id }, model, { upsert: true }, function ( err, doc ) {
